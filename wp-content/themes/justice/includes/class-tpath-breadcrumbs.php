...
if ( ! class_exists( 'Justice_Breadcrumbs' ) ) {
	class Justice_Breadcrumbs {
		...
		/**
		 * @var	string Separator between single breadcrumbs
		 */			
		private $separator;		
		...
		public function __construct() {
			...
			$defaults = array(
				 ...
				 'show_terms'  				=> 1,
				 'home_label'      			=> __( 'Home', 'Templatepath' ),
				 'tag_archive_prefix'   	=> __( 'Tag:', 'Templatepath' ),
				 'search_prefix'        	=> __( 'Search:', 'Templatepath' ),
				 'error_prefix' 			=> __( '404 - Page not Found', 'Templatepath' ),
			);
			
			// Setup a filter for changeable variables and meger it with the defaults
			$args 		= apply_filters( 'justice_breadcrumbs_defaults', $defaults );
		 ...
		 * @return void
		 */				
		public function justice_display_breadcrumbs() {
			...
			echo sprintf( '<div class="tpath-breadcrumbs">%s</div>', $this->breadcrumb_markup );
		...
		private function get_breadcrumb_home() {
			...
			// If the home page is the main blog page
				...
				$blog_title = '';
				if( isset( $this->options['tpath_blog_title'] ) && $this->options['tpath_blog_title'] == '' ) {
					$blog_title = __( 'Home', 'Templatepath' );
				} else {
					$blog_title = $this->options['tpath_blog_title'];
				}
		...
		private function get_post_terms() {
			...
			if ( $this->post->post_type == 'post' ) {
				$taxonomy = 'category';
			// Portfolio
			} elseif ( $this->post->post_type == 'tpath_portfolio' ) {
				$taxonomy = 'portfolio_categories';
			// Testimonials
			} elseif ( $this->post->post_type == 'tpath_testimonial' ) {
		...
		private function get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
			...
			if ( $separator ) {
				$separator_markup = sprintf( '<span class="tpath-breadcrumb-sep">%s</span>', $this->separator );
			}
