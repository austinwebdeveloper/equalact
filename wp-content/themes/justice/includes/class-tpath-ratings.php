<?php
/*
* The class is a modified version of the free plugin https://wordpress.org/plugins/advanced-post-type-ratings/ - Advanced Post Type Ratings by Benjamin Moody & Zeitguys Inc.
* The classname was changed to avoid any conflicts with plugins that are maybe installed
*/

class TpathPostRatings {
	
	protected static $class_config = array();
	protected $current_screen	   = NULL;
	private $plugin_ajax_nonce	   = 'tpath-post-ratings-ajax-nonce';	
	...
	function __construct( $config = array() ) {
		...
		//Add Ajax actions
		add_action("wp_ajax_post-rate-update", array($this, 'ajax_post_rate_update'));
	...
	*/
	...
	public function enqueue_admin_scripts() {
		...
		//Init vars
		$js_inc_path 	= TEMPLATEADMIN_URL . 'assets/js/';
		$css_inc_path 	= TEMPLATEADMIN_URL . 'assets/css/';
		
		wp_enqueue_script( 'jquery' );
		
		//Enqueue scripts for Rate It js plugin
		wp_register_script( 'tpath-rate-it', $js_inc_path . 'rate-it/jquery.rateit.min.js', array('jquery'), '1.0.22' );
		wp_enqueue_script( 'tpath-rate-it' );
		
		//Enqueue this plugin's script
		wp_register_script( 'tpath-post-ratings-script', $js_inc_path . 'tpath-post-ratings-script.js', array('tpath-rate-it'), '1.0' );
		wp_enqueue_script( 'tpath-post-ratings-script' );
		
		//Enqueue stylesheet for rate it plugin - can be de-enqueued and replaced by themes :)
		wp_enqueue_style( 'tpath-post-ratings-stars', $css_inc_path . 'tpath-post-ratings-stars.css', array(), '1.0' );
	...
	*/
	private function localize_script() {
		...
		$js_vars['rateUpdateAction'] = 'post-rate-update';
		...
		$js_vars['ajaxRateUpdateErrorText'] = _x( 'There was a problem updating the rating. Please try again.', 'Alert message for users', 'Templatepath' );
		
		wp_localize_script( 'tpath-post-ratings-script', $object, $js_vars );
	}
	...
	*/
	public function add_custom_column_content( $config_option_key, $column_name, $post_ID ) {
		...
		if ($column_name == $column_slug) {  
	        ...
	        //Echo out content
	        echo apply_filters( 'tpath_post_ratings_column_stars', $output, $column_name, $option, $post_ID );
	    }
	...
	public function add_custom_meta_box( $config_option_key ) {
		...
		foreach( self::$class_config as $option ) {
			...
			if( $option['meta_key'] === $config_option_key ) {
		        ...
		        //Echo out content
		        echo apply_filters( 'tpath_post_ratings_metabox_stars', $output, $option, $post_ID );
	...
	* @author	Ben Moody
	*/
	public function ajax_post_rate_update() {
		...
		if ( !wp_verify_nonce( $_REQUEST['tpathPostRateNonce'], $nonce)) {
	    	exit();
	    } 
